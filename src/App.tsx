...
export default function App(){
 ...
 <div className="scene"><Canvas camera={{position:[0,8,7],fov:42}}><ambientLight intensity={1.15}/><directionalLight position={[4,8,5]} intensity={3}/><pointLight position={[-5,4,-4]} intensity={1.5}/><Environment preset="sunset"/><Board game={game} onMove={move} locked={onlineLocked}/><ContactShadows position={[0,-.15,0]} opacity={.5} scale={12} blur={2}/><OrbitControls enablePan={false} enableRotate={false} minDistance={6} maxDistance={13} minPolarAngle={0.88} maxPolarAngle={0.88} minAzimuthAngle={0.52} maxAzimuthAngle={0.52} target={[0,0,0]}/></Canvas></div>

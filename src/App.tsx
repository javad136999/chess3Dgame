...
import { createRoom, joinRoom, subscribeRoom, savePosition } from './game/online';
import './styles.css';
...
function Board({game,onMove}:{game:Chess,onMove:(g:Chess)=>void}){
 ...
 const click=(s:string)=>{const p=game.get(s as any);if(!sel){if(p?.color===game.turn())setSel(s);return}
...
export default function App(){
 const[game,setGame]=useState(newGame);const[mode,setMode]=useState<'offline'|'online'>('offline');const[room,setRoom]=useState('');const[roomId,setRoomId]=useState<string|null>(null);const[msg,setMsg]=useState('');const[timer,setTimer]=useState({w:600,b:600});const last=useRef(Date.now());
 useEffect(()=>{if(!roomId)return subscribeRoom(roomId,(r:any)=>{if(r?.fen&&r.fen!=='startpos'){try{setGame(new Chess(r.fen))}catch{}}});},[roomId]);
 ...
 const move=async(g:Chess)=>{setGame(g);if(mode==='online'&&roomId)await savePosition(roomId,g.fen(),g.turn())};
 const newOnline=async()=>{try{const r=await createRoom(crypto.randomUUID());setRoomId(r.id);setRoom(r.id);setMsg('کد اتاق را برای بازیکن دوم بفرستید')}catch(e:any){setMsg('ابتدا Supabase را تنظیم کنید')}}
 const join=async()=>{try{const r=await joinRoom(room,crypto.randomUUID());setRoomId(r.id);setMsg('وارد اتاق شدید')}catch{setMsg('اتاق پیدا نشد یا پر است')}};
 ...
 <div className="onlinebar">{mode==='online'&&<><button onClick={newOnline}>ساخت اتاق</button><input value={room} onChange={e=>setRoom(e.target.value)} placeholder="کد اتاق"/><button onClick={join}>ورود</button></>}{msg&&<span>{msg}</span>}</div>
 <section className="hud"><b>{statusText(game)}</b><span>سفید {fmt(timer.w)}</span><span>سیاه {fmt(timer.b)}</span></section>
 <div className="scene"><Canvas camera={{position:[0,8,7],fov:42}}><ambientLight intensity={1.2}/><directionalLight position={[4,8,5]} intensity={3}/><Environment preset="sunset"/><Board game={game} onMove={move}/><ContactShadows position={[0,-.15,0]} opacity={.45} scale={12} blur={2}/><OrbitControls enablePan={false} minDistance={6} maxDistance={13} maxPolarAngle={Math.PI/2.05}/></Canvas></div>
